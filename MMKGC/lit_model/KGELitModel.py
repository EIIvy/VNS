...
class KGELitModel(BaseLitModel):
    ...
    def __init__(self, model, args, logger1):
        super().__init__(model, args)
        self.args = args
        self.logger1 = logger1
        self.logger1.info(f"args:{args}")
        self.temp = 0
    def forward(self, x):
        return self.model(x)
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.args.regul:
            loss +=self.model.regularization(pos_sample)
        self.log("Train|loss", loss,  on_step=False, on_epoch=True)
        self.logger1.info(f"Train|loss:{loss.item()}")
        return loss
    ...
    def validation_epoch_end(self, results) -> None:
        ...
        self.logger1.info(f"valid:{outputs}\n")
    ...
    def test_epoch_end(self, results) -> None:
        ...
        self.log_dict(outputs, prog_bar=True, on_epoch=True)
        self.logger1.info(f'test:{outputs}\n')
